...
$driver_name_err = $driver_phone_err = "";
 
// Processing form data when form is submitted
if($_SERVER["REQUEST_METHOD"] == "POST"){
    // Validate name
    $input_driver_name = trim($_POST["driver_name"]);//ตัดช่องว่างต้อนล้ะท้าย 
    if(empty($input_driver_name)){
        $driver_name_err = "Please enter a name.";
    } else{
        $driver_name = $input_driver_name;
    }
    
    // Validate address
    $input_driver_phone = trim($_POST["driver_phone"]);
    if(empty($input_driver_phone)){
        $driver_phone_err = "Please enter an address.";     
    } else{
        $driver_phone = $input_driver_phone;
    }
    
    // Check input errors before inserting in database
    if(empty($driver_name_err) && empty($driver_phone_err)){
        // Prepare an insert statement
        $sql = "INSERT INTO driver (id ,driver_name, driver_phone) VALUES (?, ?, ?)";

        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt,"iss",$param_id,$param_driver_name, $param_driver_phone);
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                header("location: host_driver.php");
                exit();
            } else{
               //echo $param_id;
    ...
    <div class="content-container">
      <div class="section-c">
        ...
        <div class="homecontent-c">
          <div class="content">
            <div class="ct-h">
              <h2><strong>เพิ่มรถรับส่ง</strong></h2>
            </div>
            ...
            <div class="ct-info">
              ...
              <div class="wrapper">
                    <p>กรอกรายละเอียดข้างล่างเพื่อรถรับส่งลูกค้า</p>
                    <div class="header">
                      <h2>รายละเอียดคนขับรถรับส่ง</h2>
                    </div>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="input-group <?php echo (!empty($driver_name_err)) ? 'has-error' : ''; ?>">
                            <label>ชื่อ</label>
                            <input type="text" name="driver_name" value="<?php echo $driver_name; ?>">
                            <?php echo $driver_name_err; ?>
                        </div>
                        <div class="input-group <?php echo (!empty($driver_phone_err)) ? 'has-error' : ''; ?>">
                            <label>เบอร์โทรศัพท์</label>
                            <input type="tel" name="driver_phone" placeholder="XXX-XXX-XXXX" pattern="[0]{1}[0-9]{2}-[0-9]{3}-[0-9]{4}" value="<?php echo $driver_phone; ?>">
                            <?php echo $driver_phone_err; ?>
                        </div>
                        <input type="submit" class="btn" value="Submit">
                        <a href="host_driver.php" class="btn">Cancel</a>
                    </form>
                </div>
            </div>        
        </div>
